...
2. **Pre-implementation gates.** From `task.json`, `research.json`, `plan.json`:
   ...
   ```
   Gate 1 — Task confirmed:    <✓ | ✗ — task.confirmed_by_user is false | ✗ — task.json missing>
   Gate 2 — Research complete: <✓ | ✗ — coverage_pct = X | ✗ — research.json missing>
   Gate 3 — Plan exists:       <✓ | ✗ — plan.json missing>
   Gate 4 — Plan approved:     <✓ | ✗ — review.status = draft | ✗ — review.status = rejected>
   ```
