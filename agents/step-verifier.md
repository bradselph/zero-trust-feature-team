...
- Explicit comparison: "spec said: <X> · code does: <Y> · result: ✓ matches | ✗ mismatch"
...
Truth 1 — Plan-conformance: ✓
  Files changed: <list> (matches step.files exactly)
  Anchor regions only: ✓ (diff bounded)

Truth 2 — Behavior-conformance: ✓
  BC-1: <description>
    Method: <unit-test>
    Re-run: PASS
...
Truth 3 — AC-contribution: ✓
  AC-1 (<verbatim given/when/then>):
    Trace through current code:
      <file>:<line> entry → <file>:<line> branch → <file>:<line> exit
    Reaches expected `then`: ✓

Truth 4 — No-regression: ✓
  regression_checks ran: ✓ (test-engineer Phase B PASS)
  non_goals respected: ✓ (no diff outside step.files[])
  No new hazards: ✓
